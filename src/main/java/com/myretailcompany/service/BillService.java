...
public class BillService {
	...
	private void computeTotalValues(Bill bill) {
		...
		switch (checkEmpOrAffiliate(bill)) {
		case "EMP":
			discount = 30;
			break;
		...
		if (null != bill.getLineItems()) {
			...
			while (lineItemsIter.hasNext()) {
				...
				saleValue = computeValueForItem(li.getQuantity(), li.getProduct().getProductCategory(),
						li.getProduct().getRate(),discount);
		...
		totalValue = totalValue - (((int)totalValue/100)*5);
		bill.setNoOfItems(noOfItems);
		bill.setTotalDiscount(totalCost-totalValue);
	...
	private double computeValueForItem(long quantity, ProductCategory productCategory, double rate,double discount) {
		logger.debug("productCategory : " + productCategory + "  quantity = " + quantity + "  rate = " + rate);
		double saleValue = 0;
		// If product is of type Grocery then no % based discount
		if (productCategory.equals(ProductCategory.GROCERIES)) {
			saleValue = quantity * rate;
		}else{
			// if product is an non grocery item then discount which ever applicable is applied
			if (discount != 0) {
				saleValue = quantity * rate;
	...
	public Bill createBill(Bill bill) {
		...
		logger.info("Created product with id = " + bill1.getBillId());
		return bill1;
	...
	private String checkEmpOrAffiliate(Bill bill) {
		logger.info("Checking if the user is an Employee or an Affiliate ");
		String custId = String.valueOf(bill.getCustId());
		if (custId.startsWith("EMP")) {
			logger.info("User is an Employee");
			return "EMP";
		} else if (custId.startsWith("AFF")) {
			logger.info("User is an Affiliate");
			return "AFF";
		} else {
			logger.info("User is not an Employee or an Affiliate ");
			return "USER";
		}
	}
	
	private String getDatefromCustId(String CustId){
		String customerEnrollmentDate = CustId.substring(CustId.indexOf("_"),CustId.length());
